...
int main(void)
{
  ...
  while (1)
  {
	  ...
	  uint32_t pwm = ADCtoCRR(val);
	  __HAL_TIM_SetCompare(&htim3, TIM_CHANNEL_4,pwm);


	  int dut = (val*100);
	  //__HAL TIM SetCompare(&htim3, TIM CHANNEL 4, val);
	  //__HAL TIM SetCompare(htim3, TIM CHANNEL 4, pwm);
	  sprintf(buffer1, "%s%d %s%d %s%d%s \r\n","ADC:", pollADC(), "PWM:", ADCtoCRR(val), "DS:", (dut/4096), "%");
